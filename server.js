...
const upload=multer({dest:path.join(os.tmpdir(),'yoqart-upload'),limits:{fileSize:MAX_MB*1024*1024}});
const allowedMedia=u=>{try{const x=new URL(u);return ['youtube.com','www.youtube.com','m.youtube.com','youtu.be','instagram.com','www.instagram.com'].includes(x.hostname.toLowerCase())||x.hostname.toLowerCase().endsWith('.instagram.com')}catch{return false}};
const run=(cmd,args)=>new Promise((resolve,reject)=>{const p=spawn(cmd,args,{stdio:['ignore','pipe','pipe']});let out='',err='';p.stdout.on('data',d=>out+=d);p.stderr.on('data',d=>err+=d);p.on('error',reject);p.on('close',code=>resolve({code,out,err}))});
app.use(cors());
app.use(express.json({limit:'100kb'}));
app.use(express.static('public',{extensions:['html']}));
app.get('/api/health',(req,res)=>res.json({ok:true,service:'YoqArt backend',version:'2.0.0'}));
async function cleanup(...xs){for(const x of xs)await fs.rm(x,{recursive:true,force:true}).catch(()=>{});}
async function conversion(req,res,kind){const f=req.file;if(!f)return res.status(400).json({error:'Please upload a PDF file.'});const dir=path.join(os.tmpdir(),'yoqart-convert',crypto.randomUUID());await fs.mkdir(dir,{recursive:true});const out=path.join(dir,kind==='word'?'yoqart-converted.docx':'yoqart-converted.xlsx');try{const r=await run('python3',['convert.py',kind,f.path,out]);if(r.code!==0){console.error(r.err);await cleanup(dir,f.path);return res.status(502).json({error:'PDF conversion failed. Please try a different PDF.'})}await fs.stat(out);res.download(out,kind==='word'?'yoqart-converted.docx':'yoqart-converted.xlsx',()=>cleanup(dir,f.path));}catch(e){await cleanup(dir,f.path);res.status(500).json({error:'Server conversion error.'})}}
app.post('/api/convert/pdf-to-word',upload.single('file'),(req,res)=>conversion(req,res,'word'));
app.post('/api/convert/pdf-to-excel',upload.single('file'),(req,res)=>conversion(req,res,'excel'));
